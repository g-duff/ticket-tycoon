...
export const doWork = (onDate: number, capacity: Capacity, workItems: WorkItems): WorkItems =>  {
  const stories = doStories(onDate, workItems.stories, capacity.forStories);
  return {...workItems, stories};
}

const doStories = (onDate: number, stories: Story[], storyCapacity: number): Story[] => {
  ...
  const history = currentStory.status == nextStatus ?
    currentStory.history :
    [...currentStory.history, {date:onDate, fromStatus: currentStory.status, toStatus: nextStatus}]
  const workedOnStory = { ...currentStory, daysRemaining, status: nextStatus, history }
